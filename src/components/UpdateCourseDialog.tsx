...
import { Input, Flex, Textarea  } from '@chakra-ui/react';
...
const UpdateCourseDialog: FunctionComponent<UpdateCourseDialogProps> = ({ id, name: initialName, description: initialDescription, category: initialCategory, duration: initialDuration, level: initialLevel, onClose, onCourseUpdated }) => {
    ...
    return (
        <DialogRoot open={true} onOpenChange={(open) => !open && onClose()} placement="center">
        <DialogContent>
        <DialogHeader>
          ...
          <Button onClick={handleUpdateCourse} loading={loading} colorScheme="teal">Save changes</Button>
          </DialogFooter>
        </DialogContent>
      </DialogRoot>
    );
