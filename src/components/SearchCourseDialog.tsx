...
import { DialogRoot, DialogTrigger, DialogContent, DialogHeader, DialogBody, DialogFooter, DialogTitle } from './ui/dialog';
...
const SearchCourseDialog: FunctionComponent<SearchCourseProps> = ({children, onSearch}) => {
    ...
    const handleSearch = async () => {
        setLoading(true);
        try {
          const response = await fetch(`http://localhost:8081/api/v1/courses/filter?category=${category}`);
          if (!response.ok) {
  ...
  return (
      <DialogRoot open={open} onOpenChange={(e) => setOpen(e.open)} placement="center">
        ...
        <DialogContent>
          ...
          <DialogBody>
            <Flex alignItems="center" justifyContent="center" direction="column" gap={5}>
              <Input
                ...
                _invalid={{ borderColor: 'red.300', boxShadow: '0 0 0 1px red.300' }}
                mb={4}
            />
          ...
          <DialogFooter>
            <Button onClick={() => setOpen(false)} variant="ghost" color="indigo.600">Cancel</Button>
